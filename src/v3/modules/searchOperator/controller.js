...
module.exports = {
  ...
  createMultipleRecodes: async (req, res) => {
    ...
    try {
      const { searchOperatorList } = req.body;
      if (!searchOperatorList) {
        TE({ 'error': 'searchOperatorList not available.' });
      }
      const dataList = await Service.createMultipleRecodes(searchOperatorList);
