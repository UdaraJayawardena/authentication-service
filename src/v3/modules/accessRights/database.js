...
const createSingleAccessRight = async singleAccessRight => {
  ...
  const dbAccessRight = await AccessRight.findAll({ 'where': { name: accessRight.name } });
...
const createMultipleAccessRights = async (multipleAccessRights) => {
  ...
  const dbAccessRights = await AccessRight.findAll();
