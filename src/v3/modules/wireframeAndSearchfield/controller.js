...
module.exports = {
  ...
  createMultipleRecodes: async (req, res) => {
    ...
    try {
      const { wireframeAndSearchFieldList } = req.body;
      if (!wireframeAndSearchFieldList) {
        TE({ 'error': 'wireframeAndSearchFieldList not available.' });
      }
      const dataList = await Service.createMultipleRecodes(wireframeAndSearchFieldList);
