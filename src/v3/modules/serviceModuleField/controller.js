...
module.exports = {
  ...
  createMultipleRecodes: async (req, res) => {
    ...
    try {
      const { serviceModuleFieldList } = req.body;
      if (!serviceModuleFieldList) {
        TE({ 'error': 'serviceModuleFieldList not available.' });
      }
      const dataList = await Service.createMultipleRecodes(serviceModuleFieldList);
