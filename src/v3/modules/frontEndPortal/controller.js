...
module.exports = {
  ...
  createMultipleRecodes: async (req, res) => {
    ...
    try {
      const { frontEndPortalList } = req.body;
      if (!frontEndPortalList) {
        TE({ 'error': 'frontEndPortalList not available.' });
      }
      const dataList = await Service.createMultipleRecodes(frontEndPortalList);
