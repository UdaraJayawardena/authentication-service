...
module.exports = {
  ...
  createMultipleRecodes: async (req, res) => {
    ...
    try {
      const { dashboardItemList } = req.body;
      if (!dashboardItemList) {
        TE({ 'error': 'dashboardItemList not available.' });
      }
      const dataList = await Service.createMultipleRecodes(dashboardItemList);
