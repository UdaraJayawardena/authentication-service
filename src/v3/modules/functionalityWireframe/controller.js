...
module.exports = {
  ...
  createMultipleRecodes: async (req, res) => {
    ...
    try {
      const { functionalityWireFrameList } = req.body;
      if (!functionalityWireFrameList) {
        TE({ 'error': 'functionalityWireFrame list not available.' });
      }
      const dataList = await Service.createMultipleRecodes(functionalityWireFrameList);
