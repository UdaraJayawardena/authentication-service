...
module.exports = {
  ...
  createMultipleRecodes: async (req, res) => {
    ...
    try {
      const { dashboardList } = req.body;
      if (!dashboardList) {
        TE({ 'error': 'cluster list not available.' });
      }
      const dataList = await Service.createMultipleRecodes(dashboardList);
  ...
  updateRecode: async (req, res) => {
    ...
    try {
      ...
      const transaction = await Service.updateRecode(query, updates, frontEndPortalId);
