...
module.exports = {
  ...
  createSingleRecode: async (data) => {
    ...
    if (!('backendServiceId' in data)) {
      TE('backendServiceId required in service module.');
    }
  ...
  createMultipleRecodes: async (dataList) => {
    ...
    if (dataList.length > 0) {
      const filterdDataList = dataList.filter((el) => {
        ...
        let backEndServiceIdAvailable = true;
        if (!('name' in el)) {
          nameAvailabe = false;
        }
        if (!('backendServiceId' in el)) {
          backEndServiceIdAvailable = false;
        }

        return nameAvailabe && backEndServiceIdAvailable;
      });
