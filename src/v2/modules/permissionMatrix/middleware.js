    ...
    try {
      ...
      if (Permissions.length > 0) {
        ...
        for (const permission of Permissions) {
          const role = permission.role.name.replace(/\s/g, '');
          const cluster = permission.cluster.name.replace(/\s/g, '');

          const formattedData = {
            role: permission.role.name,
            cluster: permission.cluster.name,
            accessRight: permission.accessRight.name,
